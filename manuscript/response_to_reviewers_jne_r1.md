...
We also checked whether the conclusion depends on the standard-error convention, since the primary analysis reports four. It does not, with one qualification we state rather than suppress. Under the primary participant-clustered convention no arm improves both parameters. Under the model-based, quasi-binomial and participant-bootstrap conventions, the session-level Euclidean Alignment arm is marginally better than the primary on *both* the slope and the intercept, by 0.007 to 0.036 in tau, and it has a lower pooled estimation error under every convention (0.095 against 0.098). We therefore do not claim that no arm improves anything. We claim, and the numbers support, that no arm *restores transportability*: the intercept tau remains between 0.75 and 0.92 under every convention and every alignment arm, and the prediction interval for a cohort outside the archive remains far too wide for a mapping to be carried to a new site.

The revised manuscript states this as follows.

> Alignment did not restore transportability. Aligning the calibration epochs by Euclidean Alignment left the score almost unchanged, whether the reference covariance was estimated from the session itself (Spearman 0.986 against the unaligned score) or pooled across the cohort (0.990), and left between-cohort heterogeneity where it was: slope tau 0.419 and 0.461 against 0.432, intercept tau 0.922 and 0.941 against 0.873. Aligning the score instead of the signal was worse. Standardising each cohort's scores within that cohort raised intercept tau to 1.675 and pooled estimation error to 0.129, and a within-cohort rank transformation gave 1.562 and 0.131; for both, the prediction interval for the calibration slope in an unrepresented cohort extends below zero. Because within-cohort standardisation removes the between-cohort information about absolute score level, the calibration intercept must absorb each cohort's accuracy level unaided, which is what inflates its heterogeneity. The absolute level of the calibration score therefore carries information that does transport, and removing it makes the estimate worse rather than better.
...
We also confirmed the conclusion is not an artefact of the standard-error convention, and we report the one qualification honestly: under three of the four conventions the session-level alignment arm is marginally better than the primary on both parameters, and it has a lower pooled estimation error under all four (0.095 against 0.098). We therefore claim that no arm *restores* transportability, not that no arm improves anything. The intercept tau remains between 0.75 and 0.92 under every convention and every arm.
