...
[The evaluated score is a classifier's cross-validated discriminability on each session's calibration data, not the source studies' undocumented online decoder. That score reproducibly signals within-session accuracy, but the mapping to accuracy is cohort-specific: useful for ranking sessions locally after local validation, not for reporting accuracy elsewhere.]{.mark} [The four-cohort ALS subgroup appeared more homogeneous than the full archive, illustrating how conclusions about transportability from a small cohort set can be fragile.]{.mark}
...
That distinction determines what a calibration score can be used for. Ranking sessions within a setting, which requires only that the association hold locally, is supported. [Reporting expected accuracy in a cohort where the mapping was not developed is not supported because the new-cohort intervals across estimation error, discrimination, and skill remain too wide for dependable use (Results, Transportability). Local recalibration does not on its own repair this (Results, What Local Recalibration Costs).]{.mark}
