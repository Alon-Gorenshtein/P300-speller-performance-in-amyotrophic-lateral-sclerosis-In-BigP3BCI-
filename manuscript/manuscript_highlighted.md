...
[Retrospective secondary analysis of BigP3BCI 1.0.0, the largest evaluation of this relationship to date: 18 of 20 studies contributed 271 participants, 739 session-condition records, and 19,611 character selections. The predictor was a decoder's cross-validated discriminability on calibration epochs, one study withheld from development. Between-cohort variation was summarised by the random-effects standard deviation tau.]{.mark}

### Main Results

[The association was positive in all 18 cohorts but varied widely (r 0.190 to 0.928; participant-level r = 0.714, p < 0.001), with 12 CIs excluding zero.]{.mark} Pooled estimation error was 0.098 (95% CI 0.091 to 0.107) against a benchmark of 0.146. [Calibration did not transport: intercept tau 0.87 (unrepresented-cohort interval -1.97 to 1.85, log-odds); cohort-specific slopes varied more than tenfold (tau 0.43, interval 0.111 to 2.005). Neither alignment (signal- or score-space) nor either nonlinear boundary meaningfully reduced this heterogeneity. Local recalibration with up to 16 participants did not reliably improve on transporting; refitting both intercept and slope was significantly worse than transporting at the smallest sizes.]{.mark} [Median time per selection, a stopping-rule proxy, accounted for 66% of the between-cohort slope variance in meta-regression.]{.mark} [The four ALS cohorts gave an uncorrected slope spread of 0.223, against 0.560 overall.]{.mark}

### Significance

[The evaluated score is a classifier's cross-validated discriminability on each session's calibration data, not the source studies' undocumented online decoder. That score reproducibly signals within-session accuracy, but the mapping to accuracy is cohort-specific: useful for ranking sessions after local validation, not for reporting accuracy elsewhere.]{.mark} [The four-cohort ALS subgroup appeared more homogeneous than the full archive, illustrating how conclusions about transportability from a small cohort set can be fragile.]{.mark}
...
The significance threshold was P < .05, two-sided. [Analyses used Python 3.11.14 with NumPy 2.4.6, SciPy 1.17.1, scikit-learn 1.9.0, statsmodels 0.14.6, pandas 3.0.3 and MNE 1.12.1.]{.mark}
...
[Neither nonlinear decoder recovered the regularised linear decoder's discriminability. Mean cross-validated discriminability across all 521 extracted calibration sessions was 0.714 for the kernel approximation and 0.714 for the gradient-boosted ensemble, against 0.805 for the linear decoder, which discriminated better than both nonlinear arms in every one of the 18 cohorts contributing a transportability estimate (supplement, Table S15, whose corresponding 468-session means are 0.713, 0.713 and 0.803). The close agreement between two structurally unrelated nonlinear families provides some reassurance that this result is not specific to either family's particular modelling choice, though it does not on its own establish a performance ceiling imposed by the data. Neither improved transport: slope tau was 0.461 and 0.514 and intercept tau 1.034 and 1.162, against 0.432 and 0.873. Because these scores discriminate less well than the linear one, their higher heterogeneity is not on its own evidence about nonlinear boundaries; what the comparison supports is the narrower statement that a nonlinear boundary improves neither discrimination nor transport here: the transportability failure was not rescued by either nonlinear alternative tested.]{.mark}

### What Local Recalibration Costs

[Local recalibration did not reliably improve on transporting the fitted mapping at any local sample size this archive can evaluate (Figure 6; supplement, S13; Tables S17 to S20). On the composition-balanced ladder, no size from 1 to 16 local participants produced an improvement whose confidence interval excluded zero, for either refit. At the smallest sizes the two-parameter refit was significantly worse than transporting: at two local participants it increased mean absolute error by 0.073 (95% CI 0.035 to 0.111) and at three by 0.029 (0.006 to 0.053). The intercept-only refit was never significantly worse and never significantly better, and on the draws where both refits were identified it beat the two-parameter refit at every size, significantly so at six local participants and below (supplement, Table S22); moving from -0.012 (-0.032 to +0.008) at one local participant to +0.005 (-0.007 to +0.016) at sixteen, by which point the local set carries a median of 1,056 character selections.]{.mark}

[The observed instability appears to track participant count more closely than character-selection count. The calibration score is defined at the session level (Methods), so a local sample of participants supplies at least as many distinct predictor values as participants drawn, and often only that many, since most participants in this archive (182 of 271) contribute a single recorded session; a refit of two parameters on a local sample this small is consequently fit on very few distinct points, and at two local participants 24.1% of draws returned a negative recalibrated slope (supplement, Table S21), a mapping in which higher estimated accuracy implies lower observed accuracy. Additional character selections within an already-drawn session add weight at that session's existing predictor value rather than a new one; they can improve the precision of the observed accuracy at that value but do not supply the additional distinct point a two-parameter fit needs, which helps explain why instability decreased as participant count increased. Because this analysis varies participant count without independently varying selection count, it does not by itself isolate the two. Recalibration also failed to repair the calibration parameters themselves, which rules out the possibility that it corrected the mapping while estimation error masked the correction: an intercept-only refit cannot change the slope, since an additive shift on the log-odds scale preserves it exactly, and its effect on the intercept was never distinguishable from zero, while the two-parameter refit moved both parameters significantly away from ideal at small local sizes.]{.mark}
...
[Predictor precision differs between cohorts, and measurement error attenuates a fitted slope, so some between-cohort spread could reflect precision. Correcting for measurement error changed the estimated heterogeneity only minimally, surviving a stress test on the assumed measurement-error variance (supplement, S9), making unequal predictor precision an unlikely explanation for the failure to transport.]{.mark}

Part of the variation is attributable to protocol rather than to the cohorts themselves: the time each cohort took over a selection, the archive's only trace of the stopping rule, was associated with the two-thirds reduction in residual between-cohort variance reported above (Results, Protocol Descriptors as Moderators). That association does not restore transportability, since a mapping whose slope depends on the stopping rule still cannot be carried to a site whose stopping rule is unknown, but it means the residual spread is not an irreducible population property[; protocol timing may account for a substantial share of the observed heterogeneity and should be captured explicitly in future multi-cohort validation.]{.mark}

[The 18 development folds are themselves stable, varying far less across folds than the held-out cohort-specific slopes do (supplement, Table S12), which is evidence against development-fit noise as an explanation for the heterogeneity.]{.mark}
...
Tenth, the between-cohort variance is itself estimated with limited precision (18 cohorts of 5 to 24 participants), and [cluster-robust variance estimates can be downward-biased with this number of clusters, which could inflate tau and I² (Methods, Statistical Analysis); the conclusion remained qualitatively unchanged under the participant-cluster bootstrap and at the lower confidence limit of tau (supplement, S4).]{.mark}
...
19. Collins GS, Moons KGM, Dhiman P, et al. Transparent reporting of a multivariable prediction model for individual prognosis or diagnosis statement: TRIPOD+AI. *BMJ*. 2024;385:e078378. doi:10.1136/bmj-2023-078378
