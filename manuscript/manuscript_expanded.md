...
The evaluated score is a classifier's cross-validated discriminability on each session's calibration data, not the source studies' undocumented online decoder. That score reproducibly signals within-session accuracy, but the mapping to accuracy is cohort-specific: useful for ranking sessions locally, not for reporting accuracy elsewhere. The four-cohort ALS subgroup appeared more homogeneous than the full archive, illustrating how conclusions about transportability from a small cohort set can be fragile.
