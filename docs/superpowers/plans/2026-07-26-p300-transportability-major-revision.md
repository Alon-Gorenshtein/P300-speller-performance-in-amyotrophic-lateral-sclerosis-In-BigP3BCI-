...
**The result sits near a threshold, so treat the boundary honestly rather than rounding toward the existing claim.** The cluster-robust I-squared is 79.1, just above the 75 that separates "confirmed" from "moderate". Every cohort has fewer than 30 participants, between 5 and 24, and cluster-robust variance is downward-biased and noisy below roughly 30 clusters even with the CR1 correction that statsmodels applies. A downward-biased within-cohort variance inflates tau-squared, which is the same direction as the defect this specification was introduced to remove. **So 79.1 is an upper estimate, and the true value may fall below 75.**

Do not declare heterogeneity confirmed on 79.1 without saying this in the verdict document and in the manuscript's Methods. Add the quasi-binomial specification as a third row if it is cheap to obtain, since it lands between the two at roughly tau 0.4428 and I-squared 79.4 and its agreement with the cluster-robust figure is itself evidence. If the three specifications agree to within a few points of I-squared, say so: convergence of independent corrections is a stronger argument than any single number.
